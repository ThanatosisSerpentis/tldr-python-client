...
DOWNLOAD_CACHE_LOCATION = os.environ.get(
    "TLDR_DOWNLOAD_CACHE_LOCATION", "https://tldr-pages.github.io/assets/tldr.zip"
)
...
def get_default_language() -> str:
    default_lang = get_language_code(os.environ.get("LANG", "C"))
...
def store_page_to_cache(
    page: str, command: str, platform: str, language: str
) -> Optional[str]:
    try:
...
def get_page_for_platform(
    ...
    if USE_CACHE and have_recent_cache(command, platform, language):
        ...
        try:
            data = urlopen(
                Request(page_url, headers=REQUEST_HEADERS), context=URLOPEN_CONTEXT
            ).read()
...
def update_page_for_platform(
    command: str, platform: str, remote: str, language: str
) -> None:
...
def update_cache(language: Optional[List[str]] = None) -> None:
    ...
    try:
        ...
        if language and language != "en":
            pages_dir += "." + language
        req = urlopen(
            Request(DOWNLOAD_CACHE_LOCATION, headers=REQUEST_HEADERS),
        ...
        for entry in zipfile.namelist():
            ...
            if match:
                store_page_to_cache(
                    zipfile.read(entry), match.group(2), match.group(1), language
                )
                cached += 1
...
def create_parser() -> ArgumentParser:
    ...
    parser.add_argument(
        ...
        version="%(prog)s {} (Client Specification {})".format(
            __version__, __client_specification__
        ),
    )
...
def main() -> None:
    ...
    if options.list:
        ...
        for command in options.command:
            if os.path.exists(command):
                with open(command, encoding="utf-8") as open_file:
                    output(
                        open_file.read().encode("utf-8").splitlines(),
        ...
        try:
            ...
            result = get_page(
                command, options.source, options.platform, options.language
            )
            if not result:
                sys.exit(
                    (
